...
public class RPGCharacterControllerFREE_Modif : MonoBehaviour{
	...
	void Start(){
		...
		agent.enabled = false;
		// UBACENO
		AttackHandCollidersSetActive(false);
		AttackLegCollidersSetActive(false);
	}
	...
	public GameObject[] legColliders;

	void AttackHandCollidersSetActive(bool state){
		foreach(GameObject box in handColliders){
			box.SetActive(state);
		}
	}

	void AttackLegCollidersSetActive(bool state){
		foreach(GameObject box in legColliders){
			box.SetActive(state);
		}
	}

	void HandsDeactivate(){
		AttackHandCollidersSetActive(false);
	}
	...
	void Update(){
		...
		if(animator){
			...
			if(Input.GetButtonDown("Punch") && canAction && isGrounded){
				int rnd = Random.Range(1, 3);
				Attack(rnd);
				AttackHandCollidersSetActive(true);
				Invoke("HandsDeactivate", 1f);
			}
			if(Input.GetButtonDown("Kick") && canAction && isGrounded){
				int rnd = Random.Range(1, 3);
				AttackKick(rnd);
				AttackLegCollidersSetActive(true);
				Invoke("LegsDeactivate", 1f);
			}
